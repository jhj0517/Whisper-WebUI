...
MEDIA_EXTENSION_GRADIO = MEDIA_EXTENSION + [ext.upper() for ext in MEDIA_EXTENSION]
...
def get_media_files(folder_path, include_sub_directory=False):
    media_extensions = {ext.lower() for ext in MEDIA_EXTENSION}
    ...
    if include_sub_directory:
        for root, _, files in os.walk(folder_path):
            media_files.extend(
                ...
                if _is_media(file) and os.path.exists(os.path.join(root, file))
            )
    else:
        media_files.extend(
            os.path.join(folder_path, file) for file in os.listdir(folder_path)
            if _is_media(file) and os.path.isfile(os.path.join(folder_path, file))
        )
